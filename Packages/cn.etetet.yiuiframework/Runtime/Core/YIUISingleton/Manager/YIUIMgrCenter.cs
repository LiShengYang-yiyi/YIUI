...
{
    //管理所有继承IManager的管理器
    public partial class YIUIMgrCenter : YIUIMonoSingleton<YIUIMgrCenter>
    {
        private MgrCore g_MgrCore = new MgrCore();

        //失败的初始化队列 //可根据需求对失败的进行二次处理
        private Queue<IYIUIManager> m_FailInited = new Queue<IYIUIManager>();

        //异步注册需要管理的 管理器 如果有异步初始化方法则会调用
        //返回初始化结果 如果失败是无法加入到管理中的
        //失败的可以重复调用直到成功
        //只有注册没有对应的移除  需要移除直接Dispose释放即可
        public async ETTask<bool> Register(IYIUIManager manager)
        {
        ...
        //获取失败的管理器
        //队列出队 可能没有
        public IYIUIManager GetFailInitedMgr()
        {
        ...
        //获取当前剩余的所有
        public List<IYIUIManager> GetFailInitedMgrList()
        {
            var list  = new List<IYIUIManager>();
