...
namespace YIUIFramework.Editor
{
    ...
    public static class OdinMenuTree_Extensions
    {
                                                                          ...
                                                                          bool              flattenSubDirectories = false)
        {
            ...
            assetFolderPath = assetFolderPath.TrimEnd('/') + "/";
            IEnumerable<string> strings = ((IEnumerable<string>)AssetDatabase.GetAllAssetPaths()).Where<string>((Func<string, bool>)(x =>
                    {
            ...
            foreach (string str1 in strings)
            {
                ...
                if (assetImporter != null && type.IsInstanceOfType(assetImporter))
                {
                    ...
                    {
                        string str2 =
                                (Sirenix.Utilities.PathUtilities.GetDirectoryName(str1).TrimEnd('/') + "/").Substring(assetFolderPath.Length);
                        if (str2.Length != 0)
                            path = path.Trim('/') + "/" + str2;
                    }
