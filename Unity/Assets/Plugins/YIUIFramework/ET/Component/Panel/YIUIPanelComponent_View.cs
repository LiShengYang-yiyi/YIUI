...
namespace ET.Client
{
    ...
    public partial class YIUIPanelComponent
    {
        ...
        internal async ETTask<Entity> GetView(string resName)
        {
            ...
            using var coroutineLock = await this.Root().GetComponent<CoroutineLockComponent>().Wait(CoroutineLockType.YIUILoader, resName.GetHashCode());
        ...
        internal async ETTask<Entity> GetView<T>() where T : Entity
        {
            ...
            using var coroutineLock = await this.Root().GetComponent<CoroutineLockComponent>().Wait(CoroutineLockType.YIUILoader, typeof(T).GetHashCode());
