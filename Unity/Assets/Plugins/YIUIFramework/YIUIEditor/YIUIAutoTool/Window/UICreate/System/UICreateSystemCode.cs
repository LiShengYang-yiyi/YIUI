...
{
    public class UICreateSystemCode : BaseTemplate
    {
        ...
        public UICreateSystemCode(out bool result, string authorName, UICreateSystemData codeData) : base(authorName)
        {
            var path     = $"{UIStaticHelper.UIETSystemPath}/{codeData.PkgName}/{codeData.ResName}ComponentSystem.cs";
            var template = $"{UIStaticHelper.UITemplatePath}/UICreateSystemTemplate.txt";
